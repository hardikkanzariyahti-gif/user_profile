...
function cleanupAIPayload(
  ...
  const normalizeStr = (s: string) => String(s || '').trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');

  // 1. Clean Objects with Strict Confidence Threshold (Fix 1: >= 0.60)
  const cleanObjNames = new Set<string>();
  const cleanObjectsArr: any[] = [];

  objects.forEach(obj => {
    const confidence = typeof obj === 'object' && obj !== null ? Number(obj.confidence ?? 0) : 1.0;
    if (confidence < 0.60) return; // Filter weak object detections

    const rawName = typeof obj === 'string' ? obj : (obj.name || '');
    const baseNorm = normalizeStr(rawName);

    if (!baseNorm || baseNorm === 'person' || baseNorm === 'people') return;

    const mapped = synonymMap[baseNorm] || baseNorm;
    if (cleanObjNames.has(mapped)) return;
  ...
  // 2. Clean OCR
  const cleanOcrSet = new Set<string>();
  ocr.forEach(txt => {
    const val = String(txt || '').trim();
    if (val.length <= 1) return; // Skip single char
    if (/^\d+$/.test(val.replace(/[^0-9]/g, ''))) return; // Skip pure numbers
    cleanOcrSet.add(val);
  });
  const cleanOcrArr = Array.from(cleanOcrSet);

  // 3. Clean Scenes with Strict Confidence Threshold (Fix 1: >= 0.45)
  const cleanScenesArr: any[] = [];
  ...
  sortedScenes.forEach(s => {
    ...
    if (conf < 0.45) return; // Filter weak scene detections

    const lab = normalizeStr(s.label);
    if (!lab || seenSceneLabels.has(lab)) return;
...
function enrichMetadataWithHashtagFallbacks(
  hashtags: string[],
  existingObjects: any[] = [],
  existingScenes: any[] = [],
  existingPeopleCount: number = 0,
  ...
  const objectNames = new Set<string>(existingObjects.map(o => {
    const name = typeof o === 'string' ? o : (o.name || '');
    const norm = name.toLowerCase().trim();
    return synonymMap[norm] || norm;
  }).filter(Boolean));

  const sceneLabels = new Set<string>(existingScenes.map(s => {
  ...
  hashtags.forEach(t => {
    ...
    if (clean === 'person' || clean === '1_person') {
      if (derivedPeopleCount === 0) derivedPeopleCount = 1;
    } else {
      const match = clean.match(/^(\d+)[_-]peop/);
      if (match) {
        const c = parseInt(match[1], 10);
        if (c > derivedPeopleCount) derivedPeopleCount = c;
      }
    }
  });
